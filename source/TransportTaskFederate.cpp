 ...
 */
std::wstring
TransportTaskFederate::convertStringToWstring(const std::string &str) {
  const std::ctype<wchar_t> &CType =
 ...
 * @return void
 */
void TransportTaskFederate::runFederate(std::string federateName,
                                        std::string fom, std::string address) {
  ///
  /// 1. create the RTIambassador
  ///
  rti1516::RTIambassadorFactory *factory = new rti1516::RTIambassadorFactory();
  std::vector<std::wstring>
      args; //!< vector args contains all necessary rti parameters
  ...
  wcout << L"Achieved sync point: " << READY_TO_RUN
        << L", waiting for federation..." << endl;
...
///////////////////////////////////////////////////////////////////////////////
/**
 * This method will get all the relevant handle information from the
 * RTIambassador
 */
void TransportTaskFederate::initializeHandles() {
  ...
  this->xaHandle =
      rtiamb->getParameterHandle(this->xHandle, convertStringToWstring("xa"));
  this->xbHandle =
      rtiamb->getParameterHandle(this->xHandle, convertStringToWstring("xb"));
}

/**
 * Blocks until the user hits enter
 */
void TransportTaskFederate::waitForUser() {
 ...
 * the federate
 */
void TransportTaskFederate::enableTimePolicy() {
  /////////////////////////////
 ...
 */
ObjectInstanceHandle TransportTaskFederate::registerObject() {
...
TransportTaskFederate::toVariableLengthData(const char *s) {
 ...
 */
void TransportTaskFederate::updateAttributeValues(
 ...
 * federates which are subscribed to it will receive a notification the next
 * time they tick(). Here we are passing only two of the three parameters we
 * could be passing, but we don't actually have to pass any at all!
 */
void TransportTaskFederate::sendInteraction() {
  ////////////////////////////////////////////////
  /// create the necessary container and values
  ////////////////////////////////////////////////
 ...
 * has been received.
 */
void TransportTaskFederate::advanceTime(double timestep) {
  /// request the advance
 ...
 */
void TransportTaskFederate::deleteObject(ObjectInstanceHandle objectHandle) {
